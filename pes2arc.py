...
def convert(infilename, arcfilename, outfilename):
    with open(infilename, 'rb') as f:
        header = f.read(16)
        with open(arcfilename, 'wb') as outf:
            ...
            outf.write(b'\x00') # magic end byte
...
def main():
    for filename in os.listdir(TESTDRIVE_SOURCE_DIR):
        ...
        if ext in [ '.PES' ]:
            ...
            convert(full_path_name, os.path.join(ARCFILES_DEST_DIR, basename+'.ARC'), basename+'.EMP')
